...
from apps.translations.models import Translatable, Translation
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            metavar="app_label",
            nargs="*",
            help=("Specify the app label(s) to synchronize the translations for."),
        )
        parser.add_argument(
            "--noinput",
            "--no-input",
        ...
        if app_labels:
            ...
            for app_label in app_labels:
                try:
                    ...
                    raise CommandError("App '{}' is not found.".format(app_label))
                else:
        ...
        if content_types:
            ...
            for content_type in content_types:
                ...
                if issubclass(model, Translatable):
                    ...
                    model_query = Q(content_type=content_type) & ~Q(field__in=trans_fields)
                else:
    ...
    def log_obsolete_translations(self, obsolete_translations):
        ...
        if self.verbosity >= 1:
            ...
            if obsolete_translations:
                ...
                self.stdout.write("Obsolete translations found for the specified fields:")

                for app_name, models in sorted(changes.items(), key=lambda x: x[0]):
                    self.stdout.write("- App: {}".format(app_name))
                    for model_name, fields in sorted(models.items(), key=lambda x: x[0]):
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "Obsolete translations will be deleted in the " "synchronization process."
                    )
                )
            else:
    ...
    def should_run_synchronization(self):
        ...
        if self.interactive:
            if hasattr(self.stdin, "isatty") and not self.stdin.isatty():
                self.stderr.write("Synchronization failed due to not running in a TTY.")
                self.stderr.write(
                ...
                try:
                    run = self.ask_yes_no(
                        ("Are you sure you want to synchronize the " "translations? [Y/n] "),
                        default="Y",
                    )
    ...
    def handle(self, *app_labels, **options):
        ...
        if obsolete_translations:
            ...
            if run_synchronization:
                ...
                self.stdout.write("Synchronization cancelled.")
                return

        self.stdout.write(self.style.SUCCESS("Synchronization successful."))
